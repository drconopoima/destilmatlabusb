function database1 = get_indices1()
%
%“get_indices1.m”. En caso de modificación o expansión de las bases 
...
fclose(fid);
[id,compound,~,~,~,~,~,~,~] = F{:};
database1 = containers.Map('KeyType', 'double', 'ValueType', 'any');
for i=1:length(id)
    compoundi = lower(compound{i});
    database1(id(i)) = compoundi;
end

save('indices1', 'database1');
