...
%
%Calcula la temperatura de burbuja (T) de una mezcla en fase líquida así como la
%composición de la primera traza de vapor(x)
%
%Parámetros
%
%Input:
...
%iter: Devuelve el número de iteraciones que fueron requeridas
    ...
    while abs(Ttotal) > 1e-2 && single_phase_ant < 7 && out_iter < 50  
        ...
        if (single_phase_ant < 4) || loop ~= 0 
            ...
            if ((nargin < 7) || (isempty(varargin{3}))) || ((soluc_trivial < 5e-5 && out_iter > 1 ) && t_semilla == 0)
                ...
                t_semilla = sum(t_semilla(t_semilla~=0).*Liquido.conc(t_semilla~=0)); %Si algún compuesto no se calculó el Tsat lo ignora
            ...
            for i=1:num_sust
                ...
                sup_y2 = sup_y2./sum(sup_y2);
                t_semilla2 = sum(t_semilla2(t_semilla2 ~= 0).*sup_y2); %Si algún compuesto no se calculó el Tsat lo ignora
            end
            if single_phase == 1;
    ...
    if flag ~= 10
        ...
        if (out_iter == max_iter) && isempty(loop) %Si el máximo número de iteraciones se alcanzó, muestra un warning, 
            s = sprintf('%f',valI);
            warning('MATLAB:IdealEdE', ['Error de convergencia en BubblePoint. El valor final de la función handle fue: ' s '.']);
            flag = 3;
        end
    else
        s = sprintf('%f',valI);
